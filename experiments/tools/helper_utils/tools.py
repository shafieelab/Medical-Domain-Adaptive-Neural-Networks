...
def validation_loss(loader, model, num_classes, logs_path, dset,loader_name='valid_source', num_iterations=0, is_training=True, ret_cm=False):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader[loader_name])
        for i in range(len(loader[loader_name])):
            data = iter_test.next()
            inputs = data[0]
            labels = data[1]
            inputs = inputs.cuda()
    ...
    with open(logs_path + '/_' + (str(num_iterations) if is_training else "Final") + '_confidence_values_.csv', mode='w') as file:
        csv_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

        csv_writer.writerow(['Image_Name', 'Prediction'] + list(map(lambda x: 'class_'+str(x)+'_conf',np.arange(num_classes)))) # ['Image_Name', 'Prediction', 'class_0_conf', 'class_1_conf']
    ...
    val_info= { "val_accuracy": round(val_accuracy,4), "val_loss": round(val_loss,4)}

    if num_classes==5 and 'embryo' in dset:

        cm_bnb, acc_2_class= class_5_to2(all_label_numpy=all_label.numpy(),predict_numpy=predict_numpy)

        val_info = {**val_info,"cm_bnb": cm_bnb, "val_acc_2_class": round(acc_2_class,4)}
