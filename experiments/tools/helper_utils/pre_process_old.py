...
class ResizeImage():
    ...
    def __call__(self, img):
        ...
        # img_numpy = cv2.cvtColor(img_numpy, cv2.COLOR_GRAY2BGR)
        # img_numpy = (img_numpy - np.min(img_numpy)) / (np.max(img_numpy) - np.min(img_numpy))

        # img = PIL.Image.fromarray(numpy.uint8(img_numpy))
...
def image_train(resize_size=224, crop_size=224, dataset=''):
    # if not alexnet:
...
def image_test(resize_size=224, crop_size=224, dataset=""):
    # if not alexnet:
    #     normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
    #                                      std=[0.229, 0.224, 0.225])
    # # else:
...
def image_test_10crop(resize_size=224, crop_size=224, alexnet=False):
    if not alexnet:
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])
    # else:
