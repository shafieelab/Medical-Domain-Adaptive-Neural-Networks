...
def train(config, dset_loaders):
    ...
    for itr in range(config["num_iterations"]):
        ...
        if itr % config["test_interval"] == 0:
            ...
            # print(itr_log)
            base_network.train(False)
            val_info = validation_loss(dset_loaders, base_network, dset=config['dataset'],
            ...
            print("Iteration: ", itr, "/", config["num_iterations"], "| Val loss: ", val_info['val_loss'],
                  "| Val Accuracy: ", val_info['val_accuracy'], )
            temp_model = nn.Sequential(base_network)
    ...
    if config['dataset'] == 'malaria':
        ...
        torch.save(best_model, osp.join(config["model_path"], "best_model_iter_{:05d}.pth.tar".format(best_itr)))
...
def test(config, dset_loaders, model_path_for_testing=None):
    ...
    if config["dataset"] == "sperm" and not config["data"]["target"]["labelled"]:
        ...
        test_info = validation_loss(dset_loaders, model, dset=config['dataset'], loader_name='test',
                                    num_classes=config["network"]["params"]["class_num"],
                                    logs_path=config['logs_path'], is_training=config['is_training'])
...
def main():
    ...
    config["prep"] = {'params_source': {"resize_size": resize_size, "crop_size": args.crop_size, "dataset": dataset},
                      'params_target': {"resize_size": resize_size, "crop_size": args.crop_size, "dataset": dataset}}
